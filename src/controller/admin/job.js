...
module.exports = class extends think.cmswing.admin {
  ...
  async indexAction() {
    ...
    const list = await this.model('recruit_user').where(map).order('id DESC').page(this.get('page') || 1, 20).countSelect();
    const html = this.pagination(list);
    this.assign('list', list);
    this.assign('pagerData', html); // 分页展示使用
    this.assign('is_adopt', this.get('is_adopt') ? this.get('is_adopt') : 0);
    this.meta_title = '用户求职';
    const recruitList = await this.model('recruit').select();
  ...
  async editAction() {
    ...
    if (this.isPost) {
      ...
      const res = await this.model('recruit_user').update(data);
      const url = '/admin/job/index';
      if (res) {
      ...
      const data = await this.model('recruit_user').find(id);
      this.assign('data', data);
      this.active = '/admin/job/index';
      const recruitList = await this.model('recruit').select();
      this.assign('recruitList', recruitList);
      this.meta_title = '受理求职信息';
      return this.display();
    }
  }
};
