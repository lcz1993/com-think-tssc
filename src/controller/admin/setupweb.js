...
module.exports = class extends think.cmswing.admin {
  ...
  constructor(ctx) {
    ...
    // 其他额外的操作
    this.db = this.model('setup_web');
    this.tactive = 'setupweb';
  }
  // 加载配置
  async loadsetup() {
    const fs = require('fs');
    const setup = await this.model('cmswing/setup_web').lists();
    const path1 = think.getPath('common', 'config');
    if (think.isDir(think.ROOT_PATH + '/src')) {
      const data = 'export default' + JSON.stringify(setup);
      fs.writeFileSync(think.ROOT_PATH + '/src/cmswing/config/setup_web.js', data);
    }
    const data1 = 'exports.__esModule = true;exports.default =' + JSON.stringify(setup);
    fs.writeFileSync(path1 + '/setup_web.js', data1);
  }
  async indexAction() {
    // 加载配置
    ...
    const type = this.config('setupweb.CONFIG_GROUP_LIST');
    const list = await this.db.where({'status': 1, 'group': id}).field('id,name,title,extra,value,remark,type').order('sort').select();
    for (const i of list) {
      if (i.type === 5) {
        await this.hook('homeUpPics', i.name, i.value, {$hook_key: i.name});
      }
      if (i.type === 6) {
        await this.hook('homeUpPic', i.name, i.value, {$hook_key: i.name});
      }
    }
    if (list) {
  ...
  async groupdataAction() {
    if (this.isGet) {
      ...
      lists.data.forEach(v => {
        if (v.group) {
          v.group = (this.config('setupweb.CONFIG_GROUP_LIST'))[v.group];
        } else {
          v.group = '未分组';
        }
        v.type = (this.config('setupweb.CONFIG_TYPE_LIST'))[v.type];
      });
  ...
  async addAction() {
    if (this.isPost) {
      ...
      if (addres) {
        await this.cache('setupweb', null);
      ...
      this.active = 'admin/setupweb/group';
